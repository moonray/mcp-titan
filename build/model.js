...
export class TitanMemoryModel {
    ...
    computeAttention(query, keys, values) {
        return tf.tidy(() => {
            ...
            const scores = tf.matMul(tf.matMul(query, this.queryProjection), tf.matMul(keys, this.keyProjection).transpose()).div(tf.sqrt(tf.scalar(this.keyDim)));
            ...
            const output = tf.matMul(weightedValues.reshape([-1, this.attentionHeads * this.valueDim]), this.outputProjection);
            return {
    ...
    forward(x, memoryState) {
        ...
        return tf.tidy(() => {
            ...
            const attention = this.computeAttention(encodedInput, tf.concat([encodedShortTerm, encodedLongTerm], 0), tf.concat([shortTerm, longTerm], 0));
            // Generate prediction
    ...
    trainStep(x_t, x_next, memoryState) {
        ...
        return tf.tidy(() => {
            ...
            const surpriseLoss = tf.add(tf.mul(tf.scalar(0.1), unwrapTensor(memoryUpdate.surprise.immediate)), tf.mul(tf.scalar(0.05), unwrapTensor(memoryUpdate.surprise.accumulated)));
            // Total loss
    ...
    pruneMemory(memoryState, threshold) {
        return tf.tidy(() => {
            ...
            const scores = tf.sigmoid(tf.matMul(tf.concat([shortTerm, longTerm, meta], 1), this.pruningNetwork));
